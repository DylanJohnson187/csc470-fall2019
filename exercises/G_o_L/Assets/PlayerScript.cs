...
public class PlayerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this.transform.position = new Vector3(GM.sx, 1, GM.sy); 
    }
    ...
    {
        Debug.Log("Hit A Trigger" + other.gameObject.tag);
    }

    public bool winCondition()
    {
        bool boolin_flag = false;
        for (int x = 0; x < GM.gridWidth-1; x++)
        {
            for (int y = 0; y < GM.gridHeight-1; y++)
            {
                if ((this.transform.position.x >= GM.grid[x,y].transform.position.x && this.transform.position.x < GM.grid[x + 1, y].transform.position.x) && 
                    (this.transform.position.y >= GM.grid[x, y].transform.position.y && this.transform.position.y < GM.grid[x, y +1].transform.position.y)
                        && GM.grid[x,y].goal)
                {
                    boolin_flag = true;
                }
