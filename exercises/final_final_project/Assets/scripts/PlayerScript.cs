...
public class PlayerScript : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if(objectCollided == "Obstacle" && canCollide)
        {
            //trigger end of game
        ...
        else if(objectCollided == "bluebook")
        {
            GetComponent<scorescript>().score += 10;
            Destroy(collision.gameObject);
        }
        else if (objectCollided == "redbook")
        {
            GetComponent<scorescript>().score += 25;
            Destroy(collision.gameObject);
        }
        else if (objectCollided == "goldbook")
        {
            GetComponent<scorescript>().score += 50;
            Destroy(collision.gameObject);
        }
        else if (objectCollided == "aplus")
        {
            Destroy(collision.gameObject);
            GetComponent<scorescript>().score += 100;
    ...
    {
        //okay so we cant do the boolean idea
        canCollide = false;
    }
